...
# in the data folder with (static) name "region_done_samples.csv"
read name raw_path raw_samples_table raw_classes_table data_path rest < <(sed "1d" "$1")
samples_files="$raw_path/$raw_samples_table"
done_samples="$data_path/region_done_samples.csv"
sub_samples="$raw_path/region_sub_samples.csv"
...
then
    ...
    echo "TLA_region: processing ($ncases) samples in study <$1>" 
    mkdir -p log
    # run all samples in a slum array
    # >> use {$2, $3 = --graph, --redo} for optional graphing or redoing calculations 	
    steps=$(sbatch --array=1-$ncases --parsable --export=STUDY=$1,GRAPH=$graph,REDO=$redo src/tla_regions_run_sbatch.sh)
    ...
    then
        ...
        then
            ncases=$(($(numlines $samples_files)))
            echo "TLA_region: All ($ncases) samples are already processed in study <$1>"     
        else
            echo "TLA_region: Processing remaining ($ncases) samples in study <$1>" 
    
    	# create a temporary parameter file pointing to sub-sample list
            studtbl=${1/'.csv'/'_sub.csv'}    
            cp $1 $studtbl	
            sed -Ei "2 s/[^,]*/region_sub_samples.csv/3" $studtbl
    
            # run all (undone) samples in a slum array
            mkdir -p log
            # >> use {$2, $3 = --graph, --redo} for optional graphing or redoing calculations 	
            steps=$(sbatch --array=1-$ncases --parsable --export=STUDY=$studtbl,GRAPH=$graph,REDO=$redo src/tla_regions_run_sbatch.sh)
