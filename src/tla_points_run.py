""" 
    TLA_points run for a single sample:
    #######################################################################

    TLA metrics are calculated and recorded.
    
    1- Reads parameters from a study set table (only the 1st line)
    2- Local Profiles of point_statistics are generated
    3- Patches for these spatial profiles are generated using contour levels
    4- LMEs are constructed based on aundance and mixing value cutoffs set in
       setup-sum (or given by user). Patch analuysis is done

    NOTE: Aggregated statistics across samples should be calculated
    using 'TLA_run_sum' after processing all samples in the study.
"""
...
class Sample:
    ...
    def loadLMELandscape(self, redo, do_plots=False):
        ...
        """
        from tla_functions import tofloat, tobit
    ...
    def plotLMELandscape(self, out_pth):
        ...
        with warnings.catch_warnings():
            ...
            im = plotRGB(ax, raster, self.units,
                         cedges, redges, xedges, yedges, fontsiz=16,
            ...
            fig.suptitle('Sample ID: ' + str(self.sid), fontsize=22, y=.95)
            #plt.tight_layout()
            fig.savefig(os.path.join(out_pth,
                                     self.sid +'_lme_landscape' + IMGEXT),
                        bbox_inches='tight', dpi=300)
            plt.close()
            ...
            plt.tight_layout()
            fig.savefig(os.path.join(out_pth, 
                                     self.sid +'_lme_distribution' + IMGEXT),
                        bbox_inches='tight', dpi=300)
            plt.close()
            ...
            if iscoloc:
                ...
                if do_plots:  
                    ...
                    fout = os.path.join(out_pth,
                                        self.sid +'_coloc_landscape' + IMGEXT)
        ...
        if (redo or not os.path.exists(fout)):
            ...
            if isnndist:
                ...
                if do_plots:
                    # saves to png file
                    fout = os.path.join(out_pth,
                                        self.sid +'_nndist_landscape' + IMGEXT)
                    fig.savefig(fout, bbox_inches='tight', dpi=300)
                    plt.close()
        ...
        if (redo or not isfout2 or (do_zs and not isfout)):
            ...
            if isgeordG:
                ...
                if do_plots:
                    ...
                    # saves to png file
                    fout = os.path.join(out_pth, 
                                        self.sid +'_hots_landscape' + IMGEXT)
                ...
                if do_plots:
                    ...
                    # saves to png file
                    fout = os.path.join(out_pth, 
                                        self.sid +'_hots_local_landscape' + 
                                        IMGEXT)
                    fig2.savefig(fout, bbox_inches='tight', dpi=300)
                    plt.close()    
    ...
    def plotGeOrdLandscape_simple(self, icomp, comps, metric):
        """
        Plots Getis-Ord HOT function score landscape from raster
        ...
        # plots array of landscapes for these comparisons
        fig = plotDiscreteLandscape_simple(self.sid,
                                           hotarr,
                                           self.roiarr,
                                           icomp, 
                                           comp,
                                           ...
                                           self.binsiz)
        # saves to png file
        nam = self.sid +'_hots_landscape_simple_' + \
            comp + IMGEXT
        fig.savefig(os.path.join(out_pth,nam), bbox_inches='tight', dpi=300)
        plt.close()
...
def progressBar(i, n, step, Nsteps,  msg, msg_i):
    ...
    from tla_functions import printProgressBar
...
def lmeAdjacencyHeatmap(sid, adj_pairs, res_pth ):
    ...
    f.figure.savefig(os.path.join(res_pth, 
                                  sid +'_lme_adjacency_odds' + IMGEXT),
                     bbox_inches='tight', dpi=300)
    plt.tight_layout()
    plt.close()
...
def plotDiscreteLandscape(sid, raster, roi, comps, compslabs, shape,
    ...
    import pylandstats as pls
    from tla_functions import plotRGB, plotEdges
...
def plotDiscreteLandscape_simple(sid, raster, roi, idx, clab, shape, 
                                 metric, cticks_in, lims, scale, 
                                 units, binsiz):
    """
    Plot of landscape from discrete raster
...
def plotPatchHists(sid, df, res_pth):
    ...
    fig.suptitle('Sample ID: ' + str(sid), fontsize=24, y=.95)
    #plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    fig.savefig(os.path.join(res_pth, sid +'_lme_patch_metrics_hists' + 
                             IMGEXT),
                bbox_inches='tight', dpi=300)
    plt.close()
...
def plotClassHists(sid, df, res_pth):
    ...
    fig.subplots_adjust(hspace=0.4)
    fig.suptitle('Sample ID: ' + str(sid), fontsize=24, y=.95)
    fig.savefig(os.path.join(res_pth, sid +'_lme_class_metrics_hists' + 
                             IMGEXT),
                bbox_inches='tight', dpi=300)
    #plt.tight_layout()
    plt.close()
    ...
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    fig2.savefig(os.path.join(res_pth, sid +'_lme_class_coverage' + IMGEXT),
                 bbox_inches='tight', dpi=300)
    plt.close()
    ...
    *******  Main function  *******
    ...
    if (sample.num_cells > 0):
        ...
        if (GRPH):
            ...
            if ~df.loc[df['name'] == 'geordG']['drop'].values[0]:
                comps = list(df.loc[df['name'] == 'geordG'].comps.values[0])
                for i, comp in enumerate(comps):
                    sample.plotGeOrdLandscape_simple(i, comps, 'HOT Spots')
