""" 
    TLA_points setup for a single sample:
    #######################################################################
...
class Study:
    ...
    def __init__(self, study, pth):
        ...
        if (self.BLOBS or self.DTHRES > 0.0):
            if ((self.HIGH_DENS_CODE == '') or 
                (self.LOW_DENS_CODE == '') or 
                (self.FILTER_CODE == '')):
                print("ERROR: parameters missing: FILTER_CODE, " +
                      "HIGH_DENS_CODE, and LOW_DENS_CODE are ALL needed.")
                sys.exit()
...
class Slide:
    ...
    def __init__(self, i, study):
        ...
        if 'image_file' not in self.tbl:
            self.tbl.mask_file = ''
        
        if 'mask_file' not in self.tbl:
           self.tbl.mask_file == ''
    ...
    def setup_data(self, edge=0):
        ...
        if self.isimg:
            ...
            img[0:(rmax_ - rmin),
                0:(cmax_ - cmin), :] = tobit(ims[rmin:rmax_, cmin:cmax_, :])
            self.img = img
        else:
            self.img = []
    ...
    def filter_class(self, study):
        ...
        """
        from tla_functions import KDE, arrayLevelMask, tofloat, toint
...
class Sample:
    ...
    def __init__(self, i, slide):
        ...
        from tla_functions import mkdirs, tofloat, toint
    ...
    def load_data(self):
        """Load data."""
        from skimage import io
        from tla_functions import tofloat, toint, tobit
    ...
    def kde_arrays(self, redo):
        ...
        from tla_functions import toint, tofloat
            ...
            for i, c in enumerate(cla):
                ...
                if (len(aux) > 0):
                    if do_kde:
                        ...
                        del z
                    if do_nrs:
                        from tla_functions import gkern, fftconv2d, tobit
    ...
    def abumix_mask(self, kdearr, redo):
        ...
        """
        from tla_functions import tofloat
    ...
    def plot_landscape_scatter(self, lims):
        ...
        """
        from tla_functions import plotEdges, landscapeScatter
        ...
        plt.savefig(os.path.join(self.res_pth,
                                 self.sid + '_landscape_points' + IMGEXT),
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.res_pth,
                                 self.sid + '_landscape_classes' + IMGEXT),
                    bbox_inches='tight', dpi=300)
        plt.close()
        ...
        ax.set_title('Sample ID: ' + str(self.sid), fontsize=22, y=1.04)
        plt.savefig(os.path.join(self.res_pth,
                                 self.sid + '_landscape_points_detail' + 
                                 IMGEXT),
                    bbox_inches='tight', dpi=300)
        plt.close()
        ...
        for i, row in classes.iterrows():
            ...
            ax[i].contour(x, y, self.roiarr, [.50], linewidths=2, 
                          colors='black')
            ax[i].set_title(row.class_name, fontsize=20, y=1.02)
            ax[i].set_xlim([lims2[0], lims2[1]])
            ax[i].set_ylim([lims2[2], lims2[3]])
            ax[i].set_aspect('equal', adjustable='box')
        plt.suptitle('Sample ID: ' + str(self.sid), fontsize=22, y=1.04)
        plt.tight_layout()
        plt.savefig(os.path.join(self.res_pth,
                                 self.sid + '_landscape_classes_detail' + 
                                 IMGEXT),
                    bbox_inches='tight', dpi=300)
        plt.close()
    ...
    def plot_landscape_props(self, lims):
        ...
        from tla_functions import plotEdges, KDE, plotRGB
        from tla_functions import landscapeScatter, landscapeLevels
    ...
    def plot_landscape_simple(self, inx, lims):
        ...
        from tla_functions import KDE, landscapeLevels
        from tla_functions import landscapeScatter, plotEdges
    ...
    def plot_class_landscape_props(self):
        ...
        with warnings.catch_warnings():
            ...
            fig.suptitle('Sample ID: ' + str(self.sid), fontsize=24, y=.95)
            fig.savefig(os.path.join(self.res_pth,
                                     self.sid + '_abu_mix_landscape' + IMGEXT),
                        bbox_inches='tight', dpi=300)
            plt.close()
...
def abumix(x, bw, cuda=False):
    ...
    from tla_functions import tocuda
    from tla_functions import gkern
    ...
    if cuda:
        from tla_functions import cudaconv2d
...
def getCellSize(cell_arr, r):
    ...
    from tla_functions import circle, tofloat, fftconv2d
...
if __name__ == "__main__":
    ...
    # Create the parser
    my_parser = ArgumentParser(prog="tla_points_setup",
                               description="# Single Sample Pre-processing " +
                               "module for TTLA_points #",
                               allow_abbrev=False)
